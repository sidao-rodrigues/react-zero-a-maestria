...
const mockGetItemStorage = getItemStorage as jest.Mock<unknown>;
...
describe('Auth functions', () => {
  ...
  describe('logout', () => {
    it('should call navigate and unsetAuthorizationToken', () => {
      ...
      expect(navigateMock).toHaveBeenCalledWith(ELoginRoutesEnum.LOGIN);
      expect(removeItemStorage).toHaveBeenCalledWith(AUTHORIZATION_KEY);
    });
  });
  ...
  describe('verifyLoggedIn', () => {
    ...
    it('should call redirect in token empty', () => {
      ...
      expect(redirect).toHaveBeenCalledWith(ELoginRoutesEnum.LOGIN);
    });

    it('should unsetAuthorizationToken in erro request', async () => {
